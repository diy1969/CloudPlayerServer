...
class UploadController extends Controller {
    ...
    function upload() {
        ...
        move_uploaded_file($file["tmp_name"],$_SERVER['DOCUMENT_ROOT']."/music/upload/" . $name);
        $url = C('DOMAIN') . "music/upload" . "/" . $name;

        $mp3 = new \getID3();
        $fileName=$_SERVER['DOCUMENT_ROOT']."/music/upload/" . $name;
...
//保存歌曲信息
        ...
        $data[music_url] = $url;
        $imgname==null?$data[music_img_url] ='' :$data[music_img_url]=C('DOMAIN') . "music/upload" . "/" .$imgname;
        $music->add($data);
